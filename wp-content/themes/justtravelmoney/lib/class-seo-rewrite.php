...
class SEO_Rewrite {
	...
	function schedule_flush() {
		update_option( 'jtm_seo_flush_rewrite', 1 );
	}
	 ...
	 */
	function flush() {
		if ( get_option( 'jtm_seo_flush_rewrite' ) ) {

			add_action( 'shutdown', 'flush_rewrite_rules' );
			delete_option( 'jtm_seo_flush_rewrite' );
	...
	/**
	 * Update the query vars with the redirect var
	 *
	...
	function query_vars( $query_vars ) {
		$query_vars[] = 'jtm_seo_category_redirect';
	...
	function request( $query_vars ) {
		if ( isset( $query_vars['jtm_seo_category_redirect'] ) ) {
			$catlink = trailingslashit( get_option( 'home' ) ) . user_trailingslashit( $query_vars['jtm_seo_category_redirect'], 'category' );
	...
	function category_rewrite_rules() {
		...
		if ( is_array( $categories ) && array() !== $categories ) {
			foreach ( $categories as $category ) {
				...
				if ( $category->parent === $category->cat_ID ) {
					// Recursive recursion.
					$category->parent = 0;
				} elseif ( 0 !== $taxonomy->rewrite['hierarchical'] && 0 !== $category->parent ) {
		...
		$category_rewrite[ $old_base . '$' ] = 'index.php?jtm_seo_category_redirect=$matches[1]';

		return $category_rewrite;
	}
} /* End of class */

$jtm_rewrite = new SEO_Rewrite();
